...
MAX31865_RTD rtd0(MAX31865_RTD::RTD_PT100, D12, D11, D13, A6); //miso = master input slave output (SDI on nucleo), 
//RTD parameters: (ptd_type type,PinName mosi, PinName miso, PinName sclk, PinName nss); nss is chip select, negative logic
// main() runs in its own thread in the OS

//CAN
#define CAN_RTD0_TEMP 0x656

CAN can(D10, D2, 125000);

// MAX31865_RTD rtd1(MAX31865_RTD::RTD_PT100, D12, D11, D13, A4); //miso = master input slave output (SDI on nucleo), 
// MAX31865_RTD rtd2(MAX31865_RTD::RTD_PT100, D12, D11, D13, A3); //miso = master input slave output (SDI on nucleo),
// MAX31865_RTD rtd3(MAX31865_RTD::RTD_PT100, D12, D11, D13, A0); //miso = master input slave output (SDI on nucleo),
// MAX31865_RTD rtd4(MAX31865_RTD::RTD_PT100, D12, D11, D13, A7); //miso = master input slave output (SDI on nucleo),
// MAX31865_RTD rtd5(MAX31865_RTD::RTD_PT100, D12, D11, D13, A5); //miso = master input slave output (SDI on nucleo),
// MAX31865_RTD rtd6(MAX31865_RTD::RTD_PT100, D12, D11, D13, A2); //miso = master input slave output (SDI on nucleo),
// MAX31865_RTD rtd7(MAX31865_RTD::RTD_PT100, D12, D11, D13, A1); //miso = master input slave output (SDI on nucleo),
...
int main()
{
    ...
    float temp1_buffer = 0;
    float temperature1 = 0;
    float temp2_buffer = 0;
    float temperature2 = 0;
    float temp3_buffer = 0;
    float temperature3 = 0;
    float temp4_buffer = 0;
    float temperature4 = 0;
    float temp5_buffer = 0;
    float temperature5 = 0;
    float temp6_buffer = 0;
    float temperature6 = 0;
    float temp7_buffer = 0;
    ...
    while (1) {
        ...
        //if( rtd0.status( ) == 0 ) {
            ...
            if(temp0_buffer > -10.0 && temp0_buffer < 150000.0){
                temperature0 = temp0_buffer;
            }
            printf( " T0 = %f deg C \n\r",temperature0);
            double resistance0 = rtd0.resistance();
            double raw = rtd0.raw_resistance();
            printf("Raw Resistance is %f\n", raw);
            printf("Resistance0 is %f\n", resistance0);
            //printf("%f", rtd0.status());
            //osDelay(499);
        ...
        // Output on CAN
            can.write(CANMessage(CAN_RTD0_TEMP, (uint8_t*)&data, 5));
